...
$scope.createNewVersionSubmit=function(data){if(data==undefined){AdminToastService.error(i18n["js_version_error_empty_fields"],4000);return null}if(data.copyExistingVersion){data.versionLayoutId=0}$http.post("admin/api-cms-navitem/create-page-version",$.param({"layoutId":data.versionLayoutId,"navItemId":$scope.item.id,"name":data.versionName,"fromPageId":data.fromVersionPageId}),headers).then(function(response){if(response.data.error){AdminToastService.error(i18n["js_version_error_empty_fields"],4000);return null}$scope.refreshForce();AdminToastService.success(i18n["js_version_create_success"],4000)})}});zaa.controller("CopyPageController",function($scope,$http,AdminToastService,Slug){var headers={"headers":{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"}};$scope.$on("deletedNavItem",function(){$scope.isOpen=false;$scope.itemSelection=false;$scope.selection=0});$scope.NavItemController=$scope.$parent;$scope.navId=0;$scope.items=null;$scope.isOpen=false;$scope.itemSelection=false;$scope.selection=0;$scope.select=function(item){$scope.selection=item.id;$scope.itemSelection=angular.copy(item)};$scope.aliasSuggestion=function(){$scope.itemSelection.alias=Slug.slugify($scope.itemSelection.title)};$scope.loadItems=function(){$scope.navId=$scope.NavItemController.NavController.navData.id;$http.get("admin/api-cms-nav/find-nav-items",{params:{navId:$scope.navId}}).then(function(response){$scope.items=response.data;$scope.isOpen=true})};$scope.save=function(){$scope.itemSelection["toLangId"]=$scope.NavItemController.lang.id;$http.post("admin/api-cms-nav/create-from-page",$.param($scope.itemSelection),headers).then(function(response){if(response.data){AdminToastService.success(i18n["js_added_translation_ok"],3000);$scope.NavItemController.refresh()}else{AdminToastService.error(i18n["js_added_translation_error"],5000)}})}});/*
	...
	*/zaa.filter("menuparentfilter",function(){return function(input,containerId,parentNavId){var result=[];angular.forEach(input,function(value,key){if(value.parent_nav_id==parentNavId&&value.nav_container_id==containerId){result.push(value)}});return result}});zaa.filter("menuchildfilter",function(){return function(input,containerId,parentNavId){var returnValue=false;angular.forEach(input,function(value,key){if(!returnValue){if(value.id==parentNavId&&value.nav_container_id==containerId){returnValue=value}}});return returnValue}});/*
