            ...
            <div class="crud-header">
                ...
                <div class="crud-toolbar">
                    <div class="btn-group" ng-class="{'show': toggleSettings}">
                        <button class="btn btn-sm btn-link btn-square" type="button" ng-click="toggleSettings=!toggleSettings">
        ...
        <div class="tab-pane" ng-if="crudSwitchType==0" ng-class="{'active' : crudSwitchType==0}">
            ...
            <table class="table table-hover table-align-middle table-responsive table-striped mt-4">
                ...
                <tbody ng-repeat="(key, items) in data.listArray | groupBy: config.groupByField" ng-init="viewToggler[key]=true">
                    ...
                    <tr ng-repeat="(k, item) in items track by k | srcbox:config.searchString" ng-show="viewToggler[key]" <?php if ($isInline && !$relationCall): ?>ng-click="parentSelectInline(item)" <?php if ($modelSelection): ?>ng-class="{'is-selected-row': getRowPrimaryValue(item) == <?= $modelSelection?>}"<?php endif; ?> class="selectableRow"<?php endif; ?>>
                        ...
                        <td class="text-right" ng-hide="isLocked(config.tableName, item[config.pk])">
                            <?php if (count($this->context->getButtons()) > 0): ?>
                                <?php foreach ($this->context->getButtons() as $item): ?>
                                    <button type="button" class="btn btn-sm btn-link btn-square" ng-click="<?= $item['ngClick']; ?>"><i class="material-icons"><?= $item['icon']; ?></i></button>
                                <?php endforeach; ?>
                             <?php endif; ?>
                        </td>
                        <td class="text-right" ng-show="isLocked(config.tableName, item[config.pk])">
                            <small><i class="material-icons btn-square">lock_outline</i><?= Module::t('locked_info'); ?></small>
                        </td>
                    </tr>
                </tbody>
            </table>
